...
import { Client, createClient, Facility, SyslogSeverity, SyslogMessage } from './driver';
...
const makeSender = (opts: Options): ((toSend: SyslogMessage) => Promise<LogResult>) => {
    ...
    return (toSend: SyslogMessage): Promise<LogResult> => {
        ...
        const doSend = (attempts: number): Promise<LogResult> =>
            clientPromise
                .then((client: Client) => {
                    clientPending = false;
                ...
                .catch((e) => {
                    ...
                    if (retryable) {
                        if (!clientPending) {
                            ...
                            clientPromise = new Promise((rs, rj) =>
                                wait(retryWait)
                                    .then(doCreateClient)
                                    .then(rs)
                                    .catch((e) => {
...
export const createSyslogger = (opts: Options): LoggerImpl => {
    ...
    return (prep: PreparedLog) => {
        ...
        return sender(toSend).catch((e: LogResult) => {
